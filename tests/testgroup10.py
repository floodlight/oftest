...
class Grp10No110(base_tests.SimpleDataPlane):
    """
    Emergency flows will be active even after control channel reconnection
    """
     @wireshark_capture
    def runTest(self):
        logging = get_logger()
        ...
        rv = delete_all_flows(self.controller)
        self.assertEqual(rv, 0, "Failed to delete all flows")
        delete_all_flows_emer(self.controller)
        response,pkt=self.controller.poll(ofp.OFPT_ERROR, timeout=5)
        self.assertTrue(response is None, "Emergency flow delete failed") 
        #Insert an emergency flow entry 
        test_packet = simple_tcp_packet()
        match = parse.packet_to_flow_match(test_packet)
        e_flow_mod = message.flow_mod()
        ...
        rv = self.controller.message_send(e_flow_mod)
        self.assertTrue(rv != -1, "Sending flow_mod message failed")
        response=None,
        response,pkt=self.controller.poll(ofp.OFPT_ERROR, timeout=5)
        self.assertTrue(response is None, "Cannot add emergency flows")
        self.assertEqual(do_barrier(self.controller), 0, "Barrier failed")
	...
	response=None
	response,pkt=self.controller.poll(ofp.OFPT_HELLO, timeout=5)
	self.assertTrue(response is not None, "Control channel reconnection failed")
