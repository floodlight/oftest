...
class IPv6_Untagged_Unicast(base_tests.SimpleDataPlane):
    ...
    """
    def create_interface(self, iface, untagged=True):
        ...
        actions = [ ofp.action.output(iface.port) ]
        if untagged:
        ...
        if untagged:
            ...
            ofdpa_utils.enableVlanOnPort(self.controller, iface.vlan, iface.port)
        ...
        for in_iface in sorted(ifaces):
            for out_iface in sorted(ifaces):
                ...
                parsed_expected_pkt = testutils12.simple_ipv6_packet(
                       ...
                       ip_ttl = 63,
#               dl_vlan_enable=True,
#               dl_vlan=ofdpa_utils.DEFAULT_VLAN,
#               vlan_pcp=0,
...
class IPv6_Tagged_Unicast(IPv6_Untagged_Unicast):
    def runTest(self):
        ...
        for iface in ifaces:
            self.create_interface(iface, untagged=False)
        ...
        for in_iface in sorted(ifaces):
            for out_iface in sorted(ifaces):
                ...
                parsed_expected_pkt = testutils12.simple_ipv6_packet(
                       ...
                       ip_ttl = 63,
                       dl_vlan_enable=True,
                       dl_vlan=out_iface.vlan
#               pktlen=104 # 4 less than we started with, because the way simple_tcp calc's length
                       ) 
                expected_pkt = str(parsed_expected_pkt)
                logging.info("IPv6_Tagged_Unicast test, sending iface %d to %d", ifaces[in_iface], ifaces[out_iface])
                #logging.info("Expecting packet: %s" % parsed_expected_pkt.show2())
                self.dataplane.send(in_iface.port, pkt)
                oftest.dataplane.MATCH_VERBOSE=True
                verify_packets(self, expected_pkt, [out_iface.port])
